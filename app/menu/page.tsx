			...
			<div className="flex flex-col justify-between h-full">
				<div className="p-4">
					...
					>
						{CATEGORY.map((v: string, idx: number) => {
							return (
								<div
									className={`cursor-pointer h-full w-1/5 text-center mx-4 ${
										idx === selectedIndex ? 'border-b-8 border-blue-900' : ''
									}`}
									key={v}
				...
				>
					<div className="flex flex-row justify-between self-center flex-grow border-t-2 h-[100px] items-center">
						<div className="px-8">결제 금액: {cart.reduce((acc, v) => acc + realPrice(v), 0)}원</div>
						<div className="px-8">수량: {cart.reduce((acc, v) => acc + v.count, 0)}</div>
					</div>
